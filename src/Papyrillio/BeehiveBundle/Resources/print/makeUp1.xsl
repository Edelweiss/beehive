...
<!--
    mark redundant information for deletion using a placeholder (☺)
    and span cells to cover empty space
...
-->
    ...
    <xsl:template match="table:table[not(@table:name='Allgemeines')]//table:table-cell[text:p/@text:style-name = 'blTableContentNumber']">
        ...
        <xsl:variable name="numberValue" select="papy:getCellValue($number)"/>
        <xsl:variable name="lineValue" select="papy:getCellValue($line)"/>
        <xsl:choose>
            <xsl:when test="(string($numberValue) and string($lineValue)) or (not(string($numberValue)) and not(string($lineValue)))">
                <xsl:copy>
                    <xsl:apply-templates select="@*|node()"/>
                </xsl:copy>
            </xsl:when>
            <xsl:when test="not(string($numberValue))">
                <table:covered-table-cell/>
            </xsl:when>
            <xsl:when test="not(string($lineValue))">
                <xsl:copy>
                    <xsl:copy-of select="@*"/>
    ...
    <xsl:function name="papy:getCellValue">
        ...
        <xsl:choose>
            <xsl:when test="string($current) and ($current = $previous)">
                <xsl:value-of select="'☺'"/>
            </xsl:when>
            <xsl:otherwise>
